...
public interface ProductRepository extends JpaRepository<ProductEntity, String> {
    ...
    @Query(value = "SELECT p.id, p.name, p.starting_price, p.status, p.min_price, p.max_price, p.description, p.del_flag, p.seller_id, p.sub_category_id, p.auction_format_id, p.warehouse_id, p.appraiser_id, " + 
                    ...
                    "LEFT JOIN appraiser a ON p.appraiser_id = a.id " + 
                    "WHERE p.seller_id like ?1 " + 
                    "ORDER BY p.starting_price ASC", 
            countQuery = "SELECT COUNT(p.id) FROM product p WHERE p.seller_id like ?1", 
            nativeQuery = true)
    Page<ProductEntity> sortedAscByStartingPrice(Long sellerId, Pageable pageable);
    ...
    @Query(value = "SELECT p.id, p.name, p.starting_price, p.status, p.min_price, p.max_price, p.description, p.del_flag, p.seller_id, p.sub_category_id, p.auction_format_id, p.warehouse_id, p.appraiser_id, " + 
                    ...
                    "LEFT JOIN appraiser a ON p.appraiser_id = a.id " + 
                    "WHERE p.seller_id like ?1 " + 
                    "ORDER BY p.starting_price DESC", 
            countQuery = "SELECT COUNT(p.id) FROM product p WHERE p.seller_id like ?1", 
            nativeQuery = true)
    Page<ProductEntity> sortedDescByStartingPrice(Long sellerId, Pageable pageable);
