...
<script lang="ts">
    ...
    import { addSanPham } from '@/services/nhanvien/sanPhamService'
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const fetchDanhMucCons = async (maDanhMuc: number) => {
                const result = await getDanhMucConsByMaDanhMuc(maDanhMuc)
                if (result) {
            ...
            watch(selectedDanhMuc, (newMaDanhMuc) => {
                if (newMaDanhMuc) {
                    fetchDanhMucCons(Number(newMaDanhMuc))
                } else {
                    danhMucCons.value = []
                }
            })
            ...
            const handleThemMoi = async () => {
                ...
                const response = await addSanPham(formData)
                if (response.success) {
