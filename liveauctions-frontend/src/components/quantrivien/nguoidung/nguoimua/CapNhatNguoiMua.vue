    ...
    <div class="container-fluid">
        ...
        <div class="row justify-content-evenly m-0 mt-3 mb-3">
            <div class="card-header col-xl-6">
                <h5 class="card-title mb-0">Cập nhật người mua</h5>
            </div>
...
import { useRoute, useRouter } from 'vue-router'
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const nguoiMua = ref<Record<string, undefined>>({})
            ...
            const fetchNguoiDung = async () => {
                ...
                const response = await getNguoiMuaByMaNguoiDung(maNguoiDung)
                nguoiDung.value = response.nguoiDung
                nguoiMua.value = response.nguoiMua
                previewImage.value =
            ...
            const handleCapNhat = async () => {
                ...
                Object.entries(nguoiMua.value).forEach(([key, value]) => {
                    if (
                        key !== 'trangThaiXoa' &&
                        key !== 'phieuDatCocs' &&
                        key !== 'lichSuDauGias' &&
                ...
                const response = await updateNguoiMua(Number(nguoiDung.value.maNguoiDung), formData)
                if (response.success) {
                    await router.push({
                        name: 'DanhSachNguoiDungView',
