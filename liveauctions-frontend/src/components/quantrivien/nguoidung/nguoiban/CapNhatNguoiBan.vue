    ...
    <div class="container-fluid">
        ...
        <div class="row justify-content-evenly m-0 mt-3 mb-3">
            <div class="card-header col-xl-6">
                <h5 class="card-title mb-0">Cập nhật người bán</h5>
            </div>
...
<script lang="ts">
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const nguoiBan = ref<Record<string, undefined>>({})
            ...
            const fetchNguoiDung = async () => {
                ...
                const response = await getNguoiBanByMaNguoiDung(maNguoiDung)
                nguoiDung.value = response.nguoiDung
                nguoiBan.value = response.nguoiBan
                previewImage.value =
            ...
            const handleCapNhat = async () => {
                ...
                Object.entries(nguoiBan.value).forEach(([key, value]) => {
                    if (
                        key !== 'trangThaiXoa' &&
                        key !== 'thues' &&
                        key !== 'sanPhamDangKys' &&
                ...
                const response = await updateNguoiBan(Number(nguoiDung.value.maNguoiDung), formData)
                if (response.success) {
                    await router.push({
                        name: 'DanhSachNguoiDungView',
